...
#if defined(NDEBUG)
# error "Alcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x079af9f1e1664f50c107cf0a1f0521b19998cb2db8df610762cfadd9fae17371");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Alcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Alcoin Signed Message:\n";
...
{
    // Alcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Alcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Alcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Alcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Alcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Alcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1992b541256d8b196da35887e139af38ce57b4e102412b3a7768d89dca894068");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Drita FC plays for the first time in Champions League";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044a26d5aec281c17f68d8a9b3573bbe6ba26d1a5d940868f5f5b2eddc25371ac9aef206ec030fbd2790f1381bf71634720b2ba37d464f6bdf22da3b09f83848bc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524822628;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084825252;

        if (fTestNet)
        {
            block.nTime    = 1524822580;
            block.nNonce   = 385796928;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x14b43be413f620b89e98ea7c1473a633588175e03f7ffde7ce8ff7fd288f5271"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc2, 0xb3, 0xd1 }; // Alcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AlcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AlcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AlcoinMiner(CWallet *pwallet)
{
    printf("AlcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("alcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AlcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AlcoinMiner, pwallet));
}
